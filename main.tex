...
\begin{figure}[htbp]
\includegraphics[width=1\textwidth,center]{figures/rl-env/JTR env setup.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width = 0.7\hsize]{figures/rl-reward-plots/jtr-v1 100k.png}
...
\section{State Estimator Practicality Tests}
...
COULDNT USE MODELS, USING CONSTANT SPEED AND LINEAR RUDDER - HEADING RELATIONSHIP

SO THAT THE RL ENVIRONMENT CAN BE USED WHEN STATE ESTIMATOR IS IMPROVED

As explained in Core Environment Setup section, the main goal of the gym environment is to reach where the historical boat was a few minutes later, and hopefully reach there faster than the historical boat. The reward function and the observation space of the agent was refined through many iterations to achieve this main goal. The next sections will explain the final, successful versions used. The details of what was changed during each iterations, which also include performance and rendering improvements can be seen in Appendix.
...
\chapter{OpenAI Gym Environment Refinement Iterations}
